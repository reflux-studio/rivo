...
AI 阅读资料和代码，整理概念、规则、场景和验收。问题按依赖关系逐轮展开：本轮回答清楚后，再讨论依赖这些答案的问题。每个问题带上依据、候选后果和建议。

你批准完整结论后，AI 写入 `spec.md`。子代理检查需求是否清楚、模型是否成立、验收能否识别错误行为。修复并复审通过后，再请你检查文档是否准确表达了讨论结果。

### 2. 设计方案

AI 根据 Spec 设计职责、契约、数据、状态和交付切片，与你比较重要选择。你批准方案后，AI 写入 `plan.md`。
...
讨论实体关系、系统协作、流程或状态时，用 Archify 展示现状和候选方案，随讨论更新，并保留你确认过的版本。

未安装时，AI 会说明“当前环境未安装 Archify，使用 Mermaid 替代”，然后继续。替代格式按实际需要选择 ASCII、Mermaid 或 PlantUML，无需额外确认。
...
继续任务时，AI 读取文档、审阅报告和实际改动判断下一步，用户意见从会话中核对。归档保留历史，并更新引用链接。
