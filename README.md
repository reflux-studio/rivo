...
2. **从真正受影响的节点进入。** 产品功能、设计系统、技术债和异常调查不必走同一条路线；未被触达的层不生产占位产物。
...
| 设计系统迭代 | `DESIGN.md`、设计源或 `ui-design` | Designer 先核对真实 token、组件与惯例，再通知受影响的体验方案和实现；若改变产品语义，回到 Product |
| 技术债治理 | 技术债证据、明确技术目标或 `system-design` | Engineer 用可观察成本与风险确定治理目标，再进入实现；只有外显约束变化时才需要 Product / Designer 判断 |
| 软件异常 | `investigation` | 先复现和定位根因，再归类到产品预期、设计方案、技术方案、实现、环境或外部依赖，随后按依赖传播 |

这也是为什么 rivo 标准化的是**治理协议**，不是一条万能业务流水线。Product、Designer 和 Engineer 可以在同一阶段共同探索甚至并行评审，但正式决定仍有唯一负责人。
...
不是。改变产品价值或用户行为时才从 `requirement` 进入；纯体验、技术债和软件异常都有自己的入口。成员如果发现遗漏了更高层决定，会报告路由错误，而不是自行补写一份伪需求。
