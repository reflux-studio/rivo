...
需求定义与技术设计围绕一个关键问题调查、展示候选、提问并接收回应，再推进依赖该决定的内容。阶段内写作遵守已有讨论范围，不能用完整草稿与问题清单替代协作；用户直接要求草稿或明确委托时按其要求执行。候选 target 可在确认前用于 compare，ADR 可先记录为提议。启动 Hook 直接加载入口规则，阶段技能在资料调查前加载。

本次仅进行技能格式、引用、版本与 Hook 输出检查，未运行代理行为测试；协作效果通过项目实践继续验证。
...
启动 Hook 注入 using-rivo 入口全文；具体阶段与独立能力按需加载，不预先注入全部技能。子代理调度使用宿主原生能力；宿主不支持时明确降级为单代理执行与自审，不声称已完成独立审阅。
