...
# 回到你的项目
cd your-project
rivo init                                    # 建 .rivo/,和 git init 一样是显式的一步
...
`settings.json` 不进仓库,默认在 `~/.rivo/settings.json`(机器局部,`rivo agent add` 默认写这里);只在需要项目级覆盖时才用 `--local` 写 `.rivo/settings.local.json`,并自动 gitignore。
