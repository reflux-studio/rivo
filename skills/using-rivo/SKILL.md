...
4. 由当前流程技能维护宿主任务清单，按顺序完成。已有清单直接沿用，只有完成条件成立才能勾选。

首次使用或切换技能时声明一次，同一阶段无需反复声明。当前上下文已有完整技能正文时沿用；记得技能名字或读过旧版本不等于已经加载。

需求定义、技术设计和增量交付负责阶段顺序、任务清单与交接。调研要求写在需求和设计技能中，委派具体问题时使用共享调研提示词，无需加载独立调研技能。排障、绘图、测试和决策记录等方法技能沿用当前任务的进度，方法步骤不另建清单，结果交回当前任务。单独调用方法技能时，完成用户指定范围，不启动完整交付流程。
...
其他宿主按相同动作匹配其原生能力。流程进度使用宿主任务工具，文字清单只在没有此类工具时使用。技能通过技能目录发现，工具通过工具发现入口查找；工具搜索没有结果，不能证明某个技能未安装。
...
flowchart TD
    ...
    SpecOK -->|文字或文档缺陷| Spec
    SpecOK -->|需求变化| Discuss
    SpecOK -->|通过| Design[加载 technical-design，沿用现状依据、比较并验证方案]
...
调查不固定增加一种交付文档。发现用于当前讨论，获批后把必要事实和机制写入 Spec 或 plan，重要取舍写入 ADR；用户要求独立调研报告时，按指定范围交付。
...
| “先做点事，清单和声明最后补” | 先声明用途，由当前流程维护清单，再按步骤执行。 |
| “顺便把下一期也设计了” | 回到用户指定范围，后续需求只作为约束。 |
| “每加载一个方法技能，就新建一份清单” | 由当前流程维护进度，方法结果交回当前任务。 |
| “关键现状等技术设计再查” | 先查清影响本期行为的机制和限制，再讨论需求。 |
| “进入设计，重新全面调研一遍” | 沿用已有事实，只查具体缺口或验证新选择。 |
